...
public class Dijkstra2{
    ...
    private PriorityQueue<PqElement> pq;
    ...
    public double runDijkstra(Graph g, long s, long t) {
        ...
        pq.add(new PqElement(start, distTo.get(start)));

        while (!pq.isEmpty()) {
            PqElement pE = pq.poll();
            int v = pE.getVertex();
            for (Edge e : g.adj(v))
                relax(e, v, g);
            if(v==target)
    ...
    private void relaxLim(Edge e, int v, Graph g, double distLimit, int hopLimit) {
        ...
        if (distTo.get(w) > newDist) {
            // PqElement pE = new PqElement(w, distTo.get(w));
            distTo.put(w, newDist);
            // edgeTo[w] = e;
            // if(pq.contains(pE)){
            //     pq.remove(pE);
            //     pq.add(new PqElement(w, distTo.get(w)));
            // }
            // else                
            pq.add(new PqElement(w, distTo.get(w)));
    ...
    private void relax(Edge e, int v, Graph g) {
        ...
        if (distTo.containsKey(w) && distTo.get(w) > distTo.get(v) + e.weight()) {
            ...
            // edgeTo[w] = e;
            // if (pq.containsKey(w)) pq.decreaseKey(w, distTo.get(w));
            // else                
            pq.add(new PqElement(w, distTo.get(w)));
        }
        relaxedEdgesCount ++;
